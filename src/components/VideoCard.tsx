...
export default function VideoCard({ video, isLiked, onLike, onPlay, onUsernameClick }: VideoCardProps) {
  ...
  return (
    <div className="relative h-screen snap-start bg-black">
      ...
      <div className="absolute bottom-20 left-4 right-20 z-10">
        <div className="flex items-center mb-3">
          <button 
            ...
            className="w-10 h-10 rounded-full vind-gradient flex items-center justify-center mr-3 hover:scale-110 transition-all duration-200 cursor-pointer"
          >
      ...
      <div className="absolute bottom-24 right-4 flex flex-col items-center space-y-6 z-10">
        ...
        >
          <div className={`p-2 rounded-full ${isLiked ? 'bg-primary-500/20' : 'bg-black/20'} backdrop-blur-sm border border-white/10`}>
            <Heart 
              className={`w-6 h-6 ${isLiked ? 'fill-primary-500 text-primary-500' : 'text-white'} group-hover:scale-110 transition-all`}
            />
          </div>
